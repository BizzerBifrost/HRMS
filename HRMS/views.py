...
def generate_pdf_report(requests, hr, days):
    ...
    response['Content-Disposition'] = f'attachment; filename="recruitment_analytics_report_{timezone.now().strftime("%Y%m%d")}.pdf"'
    ...
    <b>Generated on:</b> {timezone.now().strftime('%B %d, %Y at %I:%M %p')}<br/>
    <b>Analysis Period:</b> Last {days} days<br/>
    <b>Data Range:</b> {(timezone.now() - timedelta(days=days)).strftime('%B %d, %Y')} - {timezone.now().strftime('%B %d, %Y')}
    """
    ...
    if position_analysis:
            ...
            if len(position_name) > 15:
                ...
                for word in words:
                    if len(current_line + " " + word) <= 15:
                        ...
                        formatted_position += (current_line + "<br/>")
                        current_line = word
        ...
        position_table = Table(position_data, colWidths=[2.5*inch, 1*inch, 1.3*inch, 1.2*inch])
    ...
    request_table = Table(request_data, colWidths=[0.5*inch, 2*inch, 1.5*inch, 1.2*inch, 1*inch, 0.7*inch, 0.8*inch])
    ...
    Report ID: RPT-{timezone.now().strftime('%Y%m%d%H%M%S')}
    """
